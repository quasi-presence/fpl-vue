<template>
  <aside
    ...
    <div>
      <div class="flex justify-between items-center -mx-6 px-6 py-4">
        <img src="/public/fpl_logo_flare.png" class="w-12">
        <h5 class="text-cyan-400 font-bold italic text-lg">Fantasy Points League</h5>
      </div>

      <div class="mt-2 text-center">
        <img src="https://picsum.photos/100/100" class="w-10 h-10 m-auto rounded-full object-cover lg:w-20 lg:h-20">
        <span class="hidden text-gray-400 lg:block">{{ store.currentUser?.email }}</span>
      </div>

      <ul class="space-y-2 tracking-wide mt-8">
        <li>
          <a href="#" aria-label="dashboard"
            class="relative px-4 py-3 flex items-center space-x-4 rounded-xl text-white bg-gradient-to-r from-cyan-500 to-cyan-300">
    ...
    <div class="px-6 -mx-6 pt-4 flex justify-between items-center border-t">
      <button v-on:click="logout" class="px-4 py-3 flex items-center space-x-4 rounded-md text-gray-600 group">
